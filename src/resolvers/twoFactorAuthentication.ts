...
const validateToken = async (email: string, token: string) => {
    await db.query(
        "CREATE OR REPLACE FUNCTION validate_token(email_var varchar(30), token_var varchar(30)) RETURNS VOID LANGUAGE plpgsql AS $$ BEGIN if EXISTS ( SELECT 1 FROM two_factor_authentication WHERE two_factor_authentication.email = email_var AND two_factor_authentication.token=token_var) then DELETE FROM two_factor_authentication WHERE email=email_var; else RAISE EXCEPTION 'Invalid token'; END if; END $$;",
        []
    )
    return db.query(
